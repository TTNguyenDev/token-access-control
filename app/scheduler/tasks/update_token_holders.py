...
async def update_token_holders() -> None:
    ...
    logging.info(f"Start update token holders for {len(tokens)} tokens")
    for token in tokens:
        holders = {}

        try:
            if token.type == TokenDB.Type.NFTCollection:
                request = await tonapi.nft.get_all_items_by_collection_address(
                    account_id=token.address
                )
                collection = await tonapi.nft.get_collection_by_collection_address(
                    account_id=token.address
                )
                ...
                for address in request.addresses:
                    holders[address.owner.address.to_raw()] = nano_to_amount(
                        int(address.balance), 9
                    )
                    found_holders += 1
            ...
            logging.error(
                f"Failed to update holders for {token.name} [{token.address}]"
            )
            continue

        except Exception as e:
            await send_message(
                bot, config.bot.DEV_ID, text=f"{e.__class__.__name__}: {e}"
            )
            logging.error(f"{e.__class__.__name__}: {e}")
            logging.error(
                f"Failed to update holders for {token.name} [{token.address}]"
        ...
        if holders:
            ...
            logging.error(
                f"Found holders: {found_holders}, Total Holders: {total_holders}"
            )
        logging.info(f"Updated holders for {token.name} [{token.address}] [{holders}]")
